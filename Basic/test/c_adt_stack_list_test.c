...
int main(int argc, char* argv[]) {
    ...
    if (argc == 1) {
        // Run all test
        char * header[] = {
            "Begin all test functions for c_adt_stack_list"
        };
        char * footer[] = {
            "Test complete for all test functions of c_adt_stack_list"
        };
        ...
        char * header[] = {
            "Begin specific test functions for c_adt_stack_list"
        };
        char * footer[] = {
            "Test complete for all specific functions of c_adt_stack_list"
        };
        printSectionBlock(header, 1);
...
void test_init_stack() {
    ...
    Stack test_stack = malloc(sizeof(Stack_s));
    printSectionBlock(header, 1);
...
void test_push_stack() {
    ...
    assert(test_stack->nodeCount == 2);

    push_stack(3, test_stack);
    assert(peek_stack(test_stack) == 3);
    assert(test_stack->nodeCount == 3);

    printf("%s complete\n", "testing multiple push");
    printDivider();

    // Test 3: Extensive push
    ...
    for (int i = 0; i < 20; i++) {
        ...
        assert(test_stack->nodeCount == i + 1);
    }
...
void test_pop_stack() {
    ...
    // Test 4: Extensive pop
    printSubSectionBlock("testing extensive pops");
    printNewLine();
...
void test_free_stack() {
    ...
    // Test 2: Free stack with multiple element
    printSubSectionBlock("testing free_stack with stack with multiple element");
    printNewLine();
    init_stack(test_stack);
    push_stack(1, test_stack);
    push_stack(2, test_stack);
    ...
    free_stack(test_stack);
    assert(test_stack->nodeCount == 0);
    printf("%s complete\n", "testing free_stack with stack with multiple element");
    printDivider();

    // Test 3: Free on extensive stack
    printSubSectionBlock("testing free_stack with stack of 100 elements");
    printNewLine();
    init_stack(test_stack);
    for (int i = 0; i < 100; i++)
        push_stack(i, test_stack);
    free_stack(test_stack);
    assert(test_stack->nodeCount == 0);
    printf("%s complete\n", "testing free_stack with stack of 100 elements");
    printDivider();
